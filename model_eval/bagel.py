...
def eval_bagel(image, prompt, edit=False, think=False):
    ...
    if edit == True:
        output_dict = inferencer(image=image, text=prompt, think=think, **inference_hyper)
    
    else:
        output_dict = inferencer(text=prompt, think=think, **inference_hyper)
    
    #print(output_dict)
    res_img = output_dict['image']
    #res_img.save('res_test.png')
    return res_img

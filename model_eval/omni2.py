...
from OmniGen2.omnigen2.pipelines.omnigen2.pipeline_omnigen2 import OmniGen2Pipeline
from OmniGen2.omnigen2.models.transformers.transformer_omnigen2 import OmniGen2Transformer2DModel
...
def load_pipeline(args: argparse.Namespace, accelerator: Accelerator, weight_dtype: torch.dtype) -> OmniGen2Pipeline:
    ...
    if args.scheduler == "dpmsolver++":
        from OmniGen2.omnigen2.schedulers.scheduling_dpmsolver_multistep import DPMSolverMultistepScheduler
...
def eval_omni2(image, prompt, edit=False):
    if image is not None:
        img_path = [image]
    else:
        img_path = []
    input_images = preprocess(img_path)
